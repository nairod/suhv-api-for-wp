 ...
 *
 * @author Dorian Nyffeler forked from Thomas Hardegger
 * @version 21.07.2025
 */
...
class SwissUnihockey_Api_Public
{
    ...
    public static function api_club_getGames($season, $club_ID, $club_shortname, $team_ID, $mode, $cache)
    {
        ...
        if (SwissUnihockey_Api_Public::suhvDown()) {
            $value = TRUE;
        }
...
//echo "Startpage: ".$startpage."<br>";
            if ($startpage != 1) { // eine Page retour wenn nicht erste
                ...
                if ($month == "6") {
                    $page = 1;
                } // during June is the schedule not ready
                $api = new SwissUnihockey_Public();
                $api_calls++;
                $details = $api->clubGames($season, $club_ID, $team_ID, $mode, array(
                    'page' => $page
            ...
            $html_head = "<table class=\"suhv-table suhv-club-getGames" . $tablepress . "\">\n";
...
// Eine Seite retour bei Page-Ende?
            ...
            if (substr_count($game_league,"Cup")>=1) {
              $cup = TRUE;
                    ...
                    /* no email*/
                    if (substr_count($game_location, "Chur") != 0) {
                        $home_run = TRUE;
                        $home_link = "Heimspiel-Link";
                    } else {
                        $home_run = FALSE;
                        $home_link = "Gastspiel-Link";
                    }
            ...
            while ($loop) {
                ...
                do {
                    ...
                    if (($game_date == "heute") or ($game_date == "gestern") or ($game_date == "morgen")) {
                        ...
                        if ($game_date == "gestern") $date_of_game = strtotime("yesterday");
                        if ($game_date == "morgen") {
                            $date_of_game = strtotime("tomorrow");
                            $loop = FALSE;
                        }
                    } else {
                        $date_parts = explode(".", $game_date); // dd.mm.yyyy in german
                    ...
                    if (substr_count($game_location, $homegame_location) != 0) {
                        $home_run = TRUE;
                    } else {
                        $home_run = FALSE;
                    }
...
//SwissUnihockey_Api_Public::log_me($items." ".$game_date." : ".$game_homeclub." vs ".$game_guestclub);
                    ...
                    if (($items <= $n_Games)) {
                        ...
                        if (($start_of_game <= $now) and ($end_of_game >= $now) and $home_run or ($game_id == $live_game_id)) {
                            ...
                            $api_game_home = new SwissUnihockey_Public();
                            $api_calls++;
                            ...
                            $api_game_away = new SwissUnihockey_Public();
                            $api_calls++;
...
*/
        ...
        if ($value == False) {
            ...
            while ($loop) {
                ...
                do {
                    ...
                    if (isset($games[$i]->cells[4]->text[1])) {
                ...
                if (($data->slider->next == NULL) or $gameID_fix) {
                    $loop = FALSE;
                }// only this loop
                else {
                    $page++;
    ...
    public static function api_team_getPlayedGames($season, $club_ID, $club_shortname, $team_ID, $mode, $cache)
    {
        ...
        if ($value == False) {
            ...
            $title = str_replace("Resultate", SwissUnihockey_Api_Public::translate("Replacements", "Resultate"), $data->title);
            $title = str_replace("Saison", SwissUnihockey_Api_Public::translate("Replacements", "Saison"), $title);
            $Gamedetails = SwissUnihockey_Api_Public::translate("Replacements", "Spieldetails");
            ...
            while ($loop) {
                ...
                do {
                    ...
                    if ($game_result != "") {
                        $html_body .= "<tr" . ($i % 2 == 1 ? ' class="alt"' : '') . "><td class=\"suhv-datetime\">" . $game_date . ", " . $game_time .
            ...
            while ($loop) {
                ...
                do {
                    ...
                    if ($game_result == "") {
                        ...
                        $html .= "<td class=\"suhv-datetime\">" . $game_date . ", " . $game_time . "</td>";
...
// Sample: https://api-v2.swissunihockey.ch/api/rankings?&team_id=427913&club_id=423403&season=2015
    ...
    public static function api_league_getGames($season, $league, $game_class, $group, $round, $mode, $cache)
    {
        ...
        if ($value == False) {
            ...
            if ($small) {
                $html .= "<h3>" . $RoundTitle . "&nbsp;&nbsp;" . $game_maplink . $game_location_name . "</a></h3>";
            } else {
