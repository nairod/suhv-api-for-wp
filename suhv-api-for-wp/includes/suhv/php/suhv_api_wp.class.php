...
if (!class_exists('Suhv_WP')) {
    ...
    class Suhv_WP
    {
        ...
        public function __construct()
        {
            ...
            if ((isset($this->options['SUHV_default_team_id'])) && ($this->options['SUHV_default_team_id'] != "")) {
                $this->team_id = $this->options['SUHV_default_team_id'];
                $this->team_id_nla = $this->team_id;
            }
        ...
        public function my_scripts_suhv()
        {
            ...
            wp_enqueue_style('suhv-api-style-css'. '?v=1');
        ...
        function api_team_getGames($atts)
        {
            ...
            if (!empty($team_id)) {
                $this->set_team($team_id); // überschreibt immer
            } else if (!isset($this->team)) {
                $this->set_team(); // verwendet Seitenvariable
            }
            if (!isset ($this->club)) $this->set_club();
            //echo "api_club_getGames";
            $season = $this->season;
...
// Add Cron
    ...
    echo "<p class='error suhv'>Es besteht eine Kollision mit einer anderen Klasse welche ebenfalls Suhv_WP heisst!</p>";
}
